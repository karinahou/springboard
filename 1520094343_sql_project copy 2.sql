...
SELECT COUNT( membercost ) #better to count facilities
	FROM country_club.Facilities
	WHERE membercost = 0
...
SELECT Facilities.name, 
	   CASE WHEN firstname != 'GUEST' THEN CONCAT(firstname, ' ', surname)
	...
	WHERE starttime LIKE '%2012-09-14%' AND #can CASE WHEN
      ((guestcost * slots > 30 AND Bookings.memid = 0) 
      	OR (membercost * slots > 30 AND Bookings.memid != 0)) 
	ORDER BY cost DESC
...
GROUP BY Facilities.name) subquery
WHERE revenue < 1000
ORDER BY revenue
